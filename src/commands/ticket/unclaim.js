...
module.exports = {
  ...
  category: 'Ticket',
  usage: '/ticketunclaim',
  description: 'Release your claim on a support ticket',
  ...
  async execute(interaction, client) {
    try {
      ...
      const result = await ticketService.unclaimTicket(interaction);
      if (result.error) return interaction.reply({ content: result.error, ephemeral: true });
      await interaction.reply({ content: '✅ Ticket unclaimed.', ephemeral: true });
    } catch (error) {
      console.error('ticketunclaim error:', error);
  ...
  async prefixExecute(message, args, client) {
    try {
      ...
      console.error('ticketunclaim prefix error:', error);
