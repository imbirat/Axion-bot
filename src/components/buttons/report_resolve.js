...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (report.status !== 'pending') {
      return interaction.reply({ content: `Report already ${report.status}.`, flags: MessageFlags.Ephemeral });
    }
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId('report_resolve:' + reportId).setLabel('Resolve').setStyle(ButtonStyle.Success).setDisabled(true),
      new ButtonBuilder()
        .setCustomId('report_dismiss:' + reportId).setLabel('Dismiss').setStyle(ButtonStyle.Secondary).setDisabled(true),
    );
